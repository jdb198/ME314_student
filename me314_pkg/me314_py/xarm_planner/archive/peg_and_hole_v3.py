...
class PegAndHole(Node):
    ...
    # Find the contact height using force feedback
    def find_contact_height(self, xy_pose, start_height, step_size=0.005, max_steps=40):
        ...
        # Define z-force threshold for surface contact detection
        # This should be higher than the hole detection threshold
        surface_force_threshold = 1.0  # Adjust based on your sensor characteristics
        ...
        for step in range(max_steps):
            ...
            if abs(self.FT_force_z) > surface_force_threshold:
    ...
    def check_insertion_complete(self):
        ...
        """
        # Define a lower threshold for detecting the hole
        hole_force_threshold = 0.5  # N - force should be BELOW this when over a hole
        
        # Check if z-force is below the threshold (indicates hole)
        if abs(self.FT_force_z) < hole_force_threshold:
    ...
    # Spiral search for hole insertion
    def spiral_search(self, center_pose, max_radius=0.02, step_size=0.002, max_attempts=30):
        """
        Performs a spiral search pattern around a center point to find the hole.
        The hole is identified by a drop in force (indicating an opening).
        ...
        """
        self.get_logger().info("Starting spiral search pattern for hole detection...")
        ...
        self.get_logger().info("Checking center position...")
        spiral_position = [cx, cy, cz, qx, qy, qz, qw]
        self.publish_pose(spiral_position)
        time.sleep(1.5)  # Give time for motion and force reading
        
        # Check if we're over the hole (indicated by LOW force)
        if self.check_insertion_complete():
            self.get_logger().info("Hole found at center position!")
            
            # Now we lower the cylinder into the hole
            self.get_logger().info("Lowering into hole...")
            insertion_pose = [cx, cy, cz - 0.05, qx, qy, qz, qw]  # Go 5cm down
        ...
        for i in range(max_attempts):
            ...
            # Update position for next attempt
            spiral_position = [
                cx + dx,
                cy + dy, 
                cz,  # Keep same z height
                qx, qy, qz, qw  # Keep same orientation
            ]
            
            # Move to new position
            self.get_logger().info(f"Checking position offset: ({dx:.4f}, {dy:.4f})")
            self.publish_pose(spiral_position)
            time.sleep(1.5)  # Wait for motion and force reading
            
            # Check if we're over the hole at this position
            if self.check_insertion_complete():
                self.get_logger().info(f"Hole found at position offset: ({dx:.4f}, {dy:.4f})")
                
                # Now we lower the cylinder into the hole
                self.get_logger().info("Lowering into hole...")
                insertion_pose = [cx + dx, cy + dy, cz - 0.05, qx, qy, qz, qw]  # Go 5cm down
                self.publish_pose(insertion_pose)
                time.sleep(2.0)
...
# Create poses for the motion sequence
    ...
    # 7. Set up pose for spiral search just above the detected surface
    node.get_logger().info(f"Block surface found at z={surface_z:.4f}, setting up spiral search...")
    
    # Position slightly above surface for spiral search
    # We'll stay just 1mm above the surface to detect the hole
    spiral_search_pose = [
        world_coords_hole[0, 0], 
        world_coords_hole[1, 0], 
        surface_z + 0.001,  # 1mm above surface to detect hole by force drop
        1.0, 0.0, 0.0, 0.0
    ]
    
    # 8. Perform spiral search for the hole
    node.get_logger().info("Starting spiral search to find hole by detecting force drop...")
    ...
    # Additional logging for insertion success
    if hole_found:
