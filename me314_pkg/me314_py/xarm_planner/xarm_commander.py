...
This version includes bounds checking for pose commands to ensure they are within
a manually defined planning scene.
"""
...
class ME314_XArm_Queue_Commander(Node):
   def __init__(self):
       ...
       # BOUNDS CHECKING
       ####################################################################
       # Define workspace bounds in millimeters
       self.x_bounds = (150, 500)   
       self.y_bounds = (-300, 300)  
       self.z_bounds = (35, 400)    
      
       # Tracking rejected commands
       ...
       self.get_logger().info(f"Workspace bounds initialized: X={self.x_bounds}mm, Y={self.y_bounds}mm, Z={self.z_bounds}mm")
   ...
   def is_pose_within_bounds(self, pose: Pose) -> bool:
       """Check if a pose is within the defined workspace bounds.
      
       Args:
           pose: The pose to check (position in meters)
          
       Returns:
           bool: True if the pose is within bounds, False otherwise
       """
       # Convert from meters to millimeters for bounds checking
       ...
       is_valid = within_x and within_y and within_z
      
       if not is_valid:
           violations = []
           if not within_x:
               violations.append(f"X ({x_mm:.1f}mm) outside [{self.x_bounds[0]}, {self.x_bounds[1]}]mm")
           if not within_y:
               violations.append(f"Y ({y_mm:.1f}mm) outside [{self.y_bounds[0]}, {self.y_bounds[1]}]mm")
           if not within_z:
               violations.append(f"Z ({z_mm:.1f}mm) outside [{self.z_bounds[0]}, {self.z_bounds[1]}]mm")
          
           self.get_logger().warn(f"POSE OUT OF BOUNDS: {', '.join(violations)}")
          
           # Publish rejected command details
           rejected_msg = String()
           rejected_msg.data = f"REJECTED: Pose [{x_mm:.1f}, {y_mm:.1f}, {z_mm:.1f}]mm - {', '.join(violations)}"
       ...
       with self.queue_lock:
           for command in msg.commands:
               if command.command_type == "pose":
                   ...
                   # Check if the pose is within the defined bounds
                   if self.is_pose_within_bounds(pose):
                       ...
                       self.get_logger().warn(
                           f"Rejected pose command: [{pose.position.x:.3f}, {pose.position.y:.3f}, {pose.position.z:.3f}] - Outside workspace bounds"
                       )
